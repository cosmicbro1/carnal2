...
# ---------- PDF Auto-Learning ----------
def learn_from_pdfs():
    """Loads PDFs, adds their text into long-term memory if not already there."""
    pdf_text = load_pdfs_from_data()
    if not pdf_text:
        return "", False
    if "pdf_knowledge" not in MEMORY:
        MEMORY["pdf_knowledge"] = ""
    # If this PDF text is new, store it
    if pdf_text not in MEMORY["pdf_knowledge"]:
        MEMORY["pdf_knowledge"] += "\n" + pdf_text
        MEMORY.setdefault("facts", []).append(f"Learned from PDF: {pdf_text[:200]}...")
        write_json(ROOT / "memory.json", MEMORY)
        print("Carnal 2.0: PDF knowledge saved to memory.json for future sessions.")
...
# ---------- Main Loop ----------
def main():
    print("Carnal 2.0 — PDF memory enabled. Commands: ':quit', ':showmem', ':remember <fact>', ':reloadpdf', ':find <word>'")

    pdf_text, learned = learn_from_pdfs()
    ...
    while True:
        ...
        if user.lower() == ":reloadpdf":
            pdf_text, learned_now = learn_from_pdfs()
            PDF_KNOWLEDGE = pdf_text
            print("CAR2: PDF reloaded and saved to memory." if learned_now else "CAR2: PDF reloaded (no new info).")
            continue
